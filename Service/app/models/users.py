...
from .rest import (Rest, api_register, Role)
...
def before_create(data, **kwargs):
    ...
    if "username" in data and data["username"] in Users.RESERVED:
...
def after_read(results, **kwargs):
    ...
    for o in results["objects"]:
        if "username" in o and o["username"] in Users.RESERVED:
            count-= 1
...
def before_update(filters, update, **kwargs):
    ...
    if "username" in filters:
        if filters["username"] in Users.RESERVED:
        ...
        if not admin:
            ...
            filters["username"] = {"$nin":Users.RESERVED}
...
def api_login():
    ...
    if Users.login(username = data["username"],password = data["password"]):
        return jsonify({"success": True})
    else:
...
def api_get_password_reset_key(username):
    ...
    """
    Rest.authenticated()
    key = Users.get_password_reset_key(username)
    return jsonify({
        "key": "%s" % key
    })
...
def api_password_reset():
    ...
    Users.password_reset(data["username"], data["password"], 
