...
def check_status(api=True):
    ...
    #   200 started
    #   
    #   set api=False to get tuple (ready(bool), msg(str))
    status = ""
    if os.path.exists(current_app.config["ACI_STARTED_FILE"]):
        logger.debug("application started flag is set")
        if api: return jsonify({"status":"started"})
        else: return (True, "started")
